    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const list = Array.isArray(d?.upcomingEvents) ? d.upcomingEvents : [];
                const mapped = list.map((e) => {
                    const { date, time } = fmtParts(e.start_time);
                    return { title: e.title || '—', date, time, icon: <EventIcon /> };
                });
    ...
    const handleAddEvent = () => {
        if (eventTitle && eventDate) {
            ...
            setUpcomingEvents((prev) => [
                ...prev,
                { title: eventTitle, date: formattedDate, time: '04:00 مساءً', icon: <EventIcon /> },
            ]);
            setEventTitle('');
            setEventDate(null);
            setOpenDialog(false);
        ...
        <Grid container spacing={3} sx={{ direction: 'rtl', width: '95%', margin: '2rem auto' }}>
            ...
            <Grid item xs={12} md={8}>
                <Paper sx={{ borderRadius: 3 }}>
                    <Box
                        sx={{
                            background: 'linear-gradient(to left, #308A9F, #35AFBC)',
                            p: 2,
                    ...
                    >
                        <Box>
                            <Typography fontWeight="bold" fontSize="1.3rem" color="white">
                                أضف حدث جديد
                            </Typography>
                            <Typography fontSize=".8rem" color="#C7C7C7">
                                قم بإنشاء حدث جديد
                            </Typography>
                        </Box>
                    ...
                    <Box sx={{ p: 2 }}>
                        <Box
                            sx={{
                                ...
                                '& .react-datepicker__week': { display: 'flex', justifyContent: 'space-between' },
                                ...
                                '& .react-datepicker__day-name': { color: '#B3B3B3' },
                                '& .react-datepicker__day-names': { display: 'flex', justifyContent: 'space-between' },
                                '& h2.react-datepicker__current-month': { padding: '.5rem 0', fontSize: '1.1rem' },
                                '& .react-datepicker__navigation--next': { top: '1rem', right: '1rem' },
                                '& .react-datepicker__navigation--previous': { top: '1rem', left: '1rem' },
                            }}
                        >
                            <DatePicker
                                selected={eventDate}
                        ...
                        <Box sx={{ mt: 3 }}>
                            <Box sx={{ backgroundColor: '#F2F2F2', p: 2, borderRadius: 2 }}>
                                <Typography fontWeight="bold" fontSize="16px" color="#308A9F">
                                    الأحداث القادمة
                                </Typography>
                            </Box>
                            ...
                            {upcomingEvents.map((event, i) => (
                                ...
                                >
                                    ...
                                    <Box>
                                        <Typography fontWeight="bold" fontSize="14px" color="#22385F">
                                            {event.title}
                                        </Typography>
                                        <Typography fontSize="12px" color="#8F929C">
            ...
            <Grid item xs={12} md={4}>
                <Paper sx={{ borderRadius: 3 }}>
                    ...
                    >
                        <Typography fontWeight="bold" fontSize="18px" color="#308A9F">
                            الإشعارات
                        </Typography>
                        <Button size="small" sx={{ color: '#308A9F', border: '1px solid #B4D4DD', borderRadius: 2, px: 2, py: 0.5 }}>
                            عرض الكل
                        </Button>
                    </Box>
                    <Box sx={{ p: 2 }}>
                        {[
                            ...
                            { title: 'إشعار الحضور', desc: 'تم تسجيل حضور جديد للطالب.', color: '#FFD54F' },
                        ].map((note, i) => (
                ...
                <Paper sx={{ borderRadius: 3, mt: 2 }}>
                    ...
                    <Box sx={{ p: 2 }}>
                        {[
                            ...
                            { title: 'إدارة المستخدم', icon: <Avatar sx={{ bgcolor: '#9575CD', width: 24, height: 24 }}>م</Avatar>, desc: 'إدارة المستخدمين وصلاحياتهم بالكامل' },
                        ].map((link, i) => (
                            <Box key={i} sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                <Box sx={{ mr: 1 }}>{link.icon}</Box>
                                <Box>
                                    <Typography fontSize="14px" color="#22385F" fontWeight="bold">
                                        {link.title}
                                    </Typography>
                                    <Typography fontSize="12px" color="#8F929C">
                                        {link.desc}
