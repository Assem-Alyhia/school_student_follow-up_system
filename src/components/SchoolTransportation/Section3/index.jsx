                ...
                <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 650 }} aria-label="قائمة المستخدمين">
                        <TableHead sx={{ backgroundColor: '#308A9F' }}> 
                            <TableRow>
                                {['id', 'path', 'carNumber', 'carType', 'gatheringPoint', 'supervisor', 'driver', 'status', 'gpsId', 'actions'].map((headCell) => (
                                    <TableCell key={headCell} sx={{ fontSize: '0.75rem' }}> {/* تصغير حجم الخط */}
                                        <TableSortLabel
                        ...
                        <TableBody>
                            {sortedRows.map((row) => (
                                <TableRow key={row.id}>
                                    <TableCell sx={{ fontSize: '0.75rem' }}>{row.id}</TableCell> {/* تصغير حجم الخط */}
                                    ...
                                    <TableCell sx={{ fontSize: '0.75rem' }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            ...
                                            <Box>
                                                <Typography sx={{ fontSize: '0.75rem' }}>{row.supervisor.name}</Typography> {/* تصغير حجم الخط */}
                                    ...
                                    <TableCell sx={{ fontSize: '0.75rem' }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            ...
                                            <Box>
                                                <Typography sx={{ fontSize: '0.75rem' }}>{row.driver.name}</Typography> {/* تصغير حجم الخط */}
                                                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.65rem' }}>
                                                    {row.driver.number}
                                                </Typography>
                                            </Box>
                                    ...
                                    <TableCell sx={{ fontSize: '0.75rem' }}>
                                        ...
                                        <IconButton aria-label="map" size="small"> {/* أيقونة الخريطة */}
