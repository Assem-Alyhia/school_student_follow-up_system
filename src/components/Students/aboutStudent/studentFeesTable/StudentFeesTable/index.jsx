...
const StudentFeesTable = () => {
    ...
    const filtered = useMemo(() => {
        ...
        return payments.filter((item) => (
            ...
            f(item?.schoolFee?.name ?? item?.school_fee?.name).includes(f(filters.feeType)) &&
    ...
    const sorted = useMemo(() => {
        ...
        const valueByKey = (row, key) => {
            switch (key) {
                ...
                case 'parent': return row.parent?.name ?? '';
                case 'feeType': return row.schoolFee?.name ?? row.school_fee?.name ?? '';
                case 'fullAmount': return row.schoolFee?.amount ?? row.school_fee?.amount ?? 0;
                case 'paidAmount': return row.amount ?? 0;
                case 'discount': return row.discount ?? 0;
                case 'discountStatus': return row.discount_status ?? '';
                case 'remaining': return row.remaining_amount ?? 0;
                case 'paymentNo': return row.payment_number ?? '';
                case 'date': return row.paid_at ?? '';
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={12} sm={6} md={3}>
                    <TextField
                        fullWidth size="small" label="بحث باسم الطالب"
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <TextField
                        fullWidth size="small" label="بحث باسم ولي الأمر"
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <TextField
                        fullWidth size="small" label="بحث بنوع الرسوم"
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <TextField
                        fullWidth size="small" label="بحث بالحالة"
                ...
                <Table>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={12} align="center">لا توجد بيانات</TableCell>
                            </TableRow>
                        ) : (
                            sorted.map((row) => (
                                <TableRow key={row.id}>
