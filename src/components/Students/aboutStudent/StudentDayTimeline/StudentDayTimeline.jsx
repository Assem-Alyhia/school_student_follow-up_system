// src/components/Students/Schedule/StudentDayTimeline.jsx
import React, { useMemo, useState } from "react";
import {
    Box, Typography, Paper, Grid, FormControl, MenuItem, Select,
    useTheme, useMediaQuery, Popover, IconButton, CircularProgress, Alert
} from "@mui/material";
...
export default function StudentDayTimeline({ studentId }) {
    ...
    // Parse schedules once data is ready
    ...
    const selectedDate = useMemo(
        () => getDateByWeekAndDay(today.getFullYear(), selectedMonth, selectedWeek, selectedDay),
    ...
    const handleEventClick = () => {
        navigate(
            `/dashboard/student-schedule-details/${studentId}/${selectedDate.getFullYear()}/${selectedDate.getMonth() + 1}/${selectedDate.getDate()}`
        );
    };
            ...
            <Paper sx={{ p: isMobile ? 1 : 3, direction: "rtl" }}>
                ...
                {isLoading && (
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2, color: "text.secondary" }}>
                        <CircularProgress size={20} /> جاري تحميل جدول اليوم...
                    </Box>
                )}
                {isError && (
