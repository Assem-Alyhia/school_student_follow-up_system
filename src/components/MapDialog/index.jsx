...
import {
    ...
    Box, Button, Divider, Stack, MenuItem, Select, Avatar
} from '@mui/material';
import {
...
const MapDialog = ({ open, handleClose, supervisorId }) => {
    ...
    useEffect(() => {
        if (supervisorId && open) {
            setError(null);
            setLocation(null);
            setSupervisor(null);

            getSupervisorLocation(supervisorId)
                .then((response) => {
            ...
            getSupervisorById(supervisorId)
                .then((res) => {
                    setSupervisor(res?.data || res);
                })
                .catch((err) => {
                    console.error('فشل في جلب بيانات المشرف:', err);
                });
        }
    }, [supervisorId, open]);
            ...
            <DialogContent dividers sx={{ pt: 0 }}>
                <Box mb={2}>
                    ...
                    <Stack direction="row" spacing={2} alignItems="center" mt={2}>
                        <Typography variant="body2" fontWeight="bold">8930 - مجمع الفرقان</Typography>
                        <Divider orientation="vertical" flexItem />
                        <Typography variant="body2" fontWeight="bold">20 طالب</Typography>
                    </Stack>
                </Box>
