...
const DailyCalendar = () => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            console.error('فشل في حذف الحدث:', e.message);
        }
    };
            ...
            <Paper sx={{ p: isMobile ? 1 : 3, bgcolor: 'white', direction: 'rtl' }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
                    ...
                    <FormControl sx={{ minWidth: 120 }}>
                        ...
                        >
                            {[0, 1, 2, 3, 4].map((week) => (
                                <MenuItem key={week} value={week}>الأسبوع {week + 1}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                            ...
                            {dayEvents.map((event) => (
                                <Grid item xs={12} key={event.id}>
                                    <Box
                                        ...
                                        onClick={() =>
                                            navigate(`/dashboard/student-schedule-details/${selectedClassroom}/${event.start_time.getFullYear()}/${event.start_time.getMonth() + 1}/${event.start_time.getDate()}`)
                                        }
                                    >
