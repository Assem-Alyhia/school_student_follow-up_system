...
export default function AddGradeModal({
    open,
    onClose,
    onCreated,                  // اختياري: يُستدعى بعد الإنشاء بنجاح (مع الـ id إن وُجد)
    title = "إنشاء نتيجة",
    initialValues,              // اختياري: ضخ قيم ابتدائية عند الفتح
}) {
    ...
    // تحميل لوائح الخيارات عند الفتح (نفس نمط EditGradeModal)
    useEffect(() => {
        if (!open) return;
        let mounted = true;
        (async () => {
    ...
    // ضبط القيم الابتدائية عند الفتح (مثل ضخ grade في موديال التعديل)
    useEffect(() => {
        if (!open) return;
        setValues({
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            const created = await createGrade(payload);   // يُفضّل أن تُرجع الـ API الكيان/المعرف
            onCreated?.(created);
            onClose?.();
        } catch (e) {
            console.error(e);
                    ...
                    <Grid container spacing={2.25} alignItems="center">
                        ...
                        <Grid item {...fieldCols}>
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField {...params} placeholder="اختر السنة الأكاديمية" sx={fieldSx} disabled={saving} />
                                )}
                            />
                        </Grid>
                        ...
                        <Grid item {...fieldCols}>
                            <FormControl fullWidth sx={fieldSx}>
                                <Select value={values.term} onChange={change("term")} displayEmpty disabled={saving}>
