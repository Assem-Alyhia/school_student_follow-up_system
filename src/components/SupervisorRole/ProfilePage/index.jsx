...
import { getSupervisorProfile } from "../../../api/Supervisor/Profile/getSupervisorProfile";
import { updateSupervisorProfile } from "../../../api/Supervisor/Profile/updateSupervisor";
...
const SupervisorProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                setLoading(true);
                const res = await getSupervisorProfile();
                const p = res?.data || {};
                const u = p?.user || {};
                setSupervisor(p);
    ...
    const supervisorId = useMemo(() => supervisor?.id, [supervisor]);

    // حفظ جزئي لحقل واحد
    const { mutate: savePartial, isLoading: saving } = useMutation({
        mutationFn: async (payload) => {
            if (!supervisorId) throw new Error("لا يوجد معرّف مشرف.");
            return updateSupervisorProfile(supervisorId, payload);
        },
        onSuccess: (data) => {
            const p = data?.data || supervisor;
            setSupervisor(p);
        ...
        return (
            <Box sx={{ p: 3, direction: "rtl", bgcolor: "#f8f9fa" }}>
                <Paper sx={{ p: 3, borderRadius: 3 }} elevation={1}>
                    <Typography>جارِ تحميل بروفايل المشرف…</Typography>
                </Paper>
            </Box>
        );
    }
