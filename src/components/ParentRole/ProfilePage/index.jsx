...
import { updateParentProfile } from "../../../api/Parent/Profile/updateParentProfile";
...
const ParentProfile = () => {
    ...
    const parentId = useMemo(() => parent?.id, [parent]);

    // حفظ جزئي لحقل واحد
    const { mutate: savePartial, isLoading: saving } = useMutation({
        mutationFn: async (payload) => {
            if (!parentId) throw new Error("لا يوجد معرّف لوليّ الأمر.");
            return updateParentProfile(parentId, payload);
        },
        onSuccess: (data) => {
        ...
        return (
            <Box sx={{ p: 3, direction: "rtl", bgcolor: "#f8f9fa" }}>
                <Paper sx={{ p: 3, borderRadius: 3 }} elevation={1}>
                    <Typography>جارِ تحميل بروفايل وليّ الأمر…</Typography>
                </Paper>
            </Box>
        );
    }
    ...
    return (
        <Box sx={{ p: 3, direction: "rtl", bgcolor: "#f8f9fa" }}>
            <Paper sx={{ p: 3, mb: 4, borderRadius: 3 }} elevation={1}>
                ...
                {/* للعرض فقط - لا أزرار */}
                <DisplayRow label="الهاتف (الحالي)" value={profileData.phone} />
